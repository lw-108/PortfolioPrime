...
class DiscGeometry extends Geometry {
  constructor(_steps = 4, radius = 1) {
    super();
    this.addVertex(0, 0, 0);
    this.lastVertex.uv[0] = 0.5;
    this.lastVertex.uv[1] = 0.5;

    this.addVertex(-radius, radius, 0);
...
class InfiniteGridMenu {
  ...
  public onActiveItemChange: ActiveItemCallback;
  public onMovementChange: MovementChangeCallback;
  ...
  private init(onInit?: InitCallback): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const x = Math.cos(theta) * Math.sin(phi) * this.SPHERE_RADIUS;
      const y = Math.sin(theta) * Math.sin(phi) * this.SPHERE_RADIUS;
      const z = Math.cos(phi) * this.SPHERE_RADIUS;
      this.instancePositions.push(vec3.fromValues(x, y, z));
    }
    this.DISC_INSTANCE_COUNT = count;
  ...
  private render(): void {
    ...
    gl.uniformMatrix4fv(this.discLocations.uWorldMatrix, false, this.worldMatrix as unknown as Float32Array);
    gl.uniformMatrix4fv(this.discLocations.uViewMatrix, false, this.camera.matrices.view as unknown as Float32Array);
    gl.uniformMatrix4fv(this.discLocations.uProjectionMatrix, false, this.camera.matrices.projection as unknown as Float32Array);
    gl.uniform3f(
        ...
        <>
          ...
          >
            <div className="flex-1 min-w-0 text-left pr-2">
              <h3 className="text-base font-bold truncate text-foreground">{activeItem.title}</h3>
              <p className="text-xs mt-1 whitespace-normal wrap-break-word leading-relaxed max-w-[90%] text-muted-foreground">{activeItem.description}</p>
            </div>
            <button
              onClick={handleButtonClick}
              className="shrink-0 w-10 h-10 rounded-full bg-[#f54900] flex items-center justify-center font-bold text-lg hover:scale-105 active:scale-95 transition-all shadow-md border-none outline-none cursor-pointer"
            >
              <p className="select-none relative text-[#ffffe3] top-px text-[18px] font-clash font-extrabold">&#x2197;</p>
            </button>
          </div>

          {/* Desktop view: floating absolute texts */}
          <h2
            className={`
              select-none
              absolute
          ...
          <button
            ...
            className={`
              ...
              ${isMoving
                ...
                : 'bottom-[2.8em] opacity-100 pointer-events-auto duration-500 scale-100 -translate-x-1/2'
              }
            `}
          >

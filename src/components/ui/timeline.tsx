...
export const Timeline = ({ data, title, description }: TimelineProps) => {
  ...
  return (
    <div
      className="w-full bg-transparent font-clash md:px-10"
      ref={containerRef}
    >
      {(title || description) && (
        <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
          {title && (
            <h2 className="text-lg md:text-4xl mb-4 text-foreground max-w-4xl font-bold tracking-tight">
              {title}
            </h2>
          )}
          {description && (
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-foreground/40 dark:text-foreground/40">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
        ...
        <div
          ...
          className="absolute md:left-8 left-8 top-0 overflow-hidden w-[2px] bg-neutral-200 dark:bg-neutral-800 mask-[linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]"
        >
          <motion.div
            style={{

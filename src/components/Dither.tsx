...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    // Direct resolution updates
    const dpr = gl.getPixelRatio();
    const newWidth = Math.floor(size.width * dpr);
    const newHeight = Math.floor(size.height * dpr);
    if (u.resolution.value.x !== newWidth || u.resolution.value.y !== newHeight) {
  ...
  const handlePointerMove = (e: ThreeEvent<PointerEvent>) => {
    ...
    const dpr = gl.getPixelRatio();
    // Since the container is mirrored horizontally with scale-x-[-1], we invert the X coordinate relative to the rect width
    mouseRef.current.set(rect.width * dpr - (e.clientX - rect.left) * dpr, (e.clientY - rect.top) * dpr);
  };
